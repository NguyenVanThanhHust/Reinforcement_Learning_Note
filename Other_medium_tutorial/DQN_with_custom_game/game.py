...
class Fruit:
    def __init__(self, height=1, width=1, x=None, y=0, speed=1, field=None) -> None:
        self.field = field
        self.height = height
        self.width = width
    ...
    def move(self):
        self.y += self.speed
        self.check_out_of_field()

    def check_is_caught(self, player):
        if self.y != player.y:
            self.is_caught = 0
        else:
...
class Field:
    def __init__(self, height=10, width=5) -> None:
        self.width = width
        self.height = height
        self.clear_field()
    ...
    def step(self, action=None):
        ...
        if self.game_tick % self.MOVE_FRUIT_EVERY == 0:
            ...
            for fruit in self.fruits:
                fruit.move()
                fruit.check_is_caught(self.player)
